...
int main(int argc,char **argv){
    ...
    while(1){
        ...
        switch(x){
            case SGE_CURSOR:
                if(w->keys&SGK_LB){
                    sgm_cfg(m,w->pixel,w->rectangle.w,w->rectangle.h);
                    sgm_circle(m,w->cursor.x,w->cursor.y,5+rand()%20,rand());
                }
                if(w->keys&SGK_RB){
                    const int rect=5+rand()%20;
                    sgm_cfg(m,w->pixel,w->rectangle.w,w->rectangle.h);
                    sgm_rect(m,w->cursor.x-(rect>>1),w->cursor.y-(rect>>1),rect,rect,rand());
                }
                break;
            case SGE_PRESS:
                printf("press %d (%c)\n",w->keys,(char)e->key);
